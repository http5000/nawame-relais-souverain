...
dans Vercel) et déchiffre le contenu de vos mémoires avant de le renvoyer à l'IA.
Nawame, elle, ne voit que du contenu chiffré : elle ne peut rien lire.
...
- Votre **passphrase souverain** (la phrase longue que vous avez choisie à
  l'activation du mode souverain dans Nawame). Si vous ne l'avez pas encore
  activé : Console Nawame › Mon compte › Mode souverain › Activer.
...
### Étape 3 — Ajouter vos deux secrets dans Vercel

Ces deux valeurs sont stockées CHEZ VOUS, dans Vercel. Elles ne nous sont jamais
...
3. Ajoutez la deuxième variable :
   ...
   - **Value** : collez votre passphrase souverain (la phrase longue).
   - **Environment** : cochez les trois.
4. Cliquez **Save**.
5. Retournez dans **Deployments**, ouvrez le dernier, cliquez **Redeploy**.
...
2. Testez-la dans votre navigateur : elle doit afficher
   `{"relay":"nawame-relais-souverain","configured":true,…}`.
   Si `configured` est `false`, vos secrets ne sont pas bons (revoir l'étape 3).
...
Vercel) and decrypts your memory content before returning it to the AI. Nawame
only sees encrypted content: it cannot read anything.
...
- Your **sovereign passphrase** (the long phrase you chose when activating
  sovereign mode in Nawame). If you haven't activated it yet: Nawame Console
  › My account › Sovereign mode › Activate.
...
### Step 3 — Add your two secrets in Vercel

These two values are stored WITH YOU, in Vercel. They are never sent to us.
...
3. Add the second variable:
   - **Name**: `SOUVERAIN_PASSPHRASE`
   - **Value**: paste your sovereign passphrase (the long phrase).
   - **Environment**: check all three.
4. Click **Save**.
5. Go back to **Deployments**, open the latest, click **Redeploy**.
...
2. Test it in your browser: it should show
   `{"relay":"nawame-relais-souverain","configured":true,…}`.
   If `configured` is `false`, your secrets are wrong (redo Step 3).
...
Template v1. The crypto core (`src/crypto-souverain.ts`) is shared with Nawame
(same code). The MCP proxy (`app/api/mcp/route.ts`) forwards requests to Nawame
with the account's API key; encrypted-content decryption hooks in here based
on the MCP response format stabilized on the Nawame side (integration point
